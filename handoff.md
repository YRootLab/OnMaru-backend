...
- 재현 계약: canonical input SHA-256과 actual을 제외한 gold SHA-256, 분자·분모, 관측값·threshold, gate 결과를 closed JSON report `1.1`에 기록한다. 실행 시각을 제외해 같은 fixture는 byte-identical report를 만들고, `--compare`는 동일 gold 여부와 metric delta를 같은 schema로 반환한다.
- 리뷰 보강: #105와 같은 board cardinality/shape를 입력 단계에서 강제하고 retrieval 중복은 입력 거절과 nDCG 방어를 모두 적용했다. reason별 생성 summary와 provider가 아닌 frozen `humanClaimSupported`를 저장해 의미 지지율을 ID 정합성과 분리했다.
...
- 검증: 신규 eval 11개, FastAPI 전체 115 passed·1 opt-in live smoke skipped, Ruff/mypy, Node 37개, contract validator 9개, planning/Odii, offline 5 gates·schema drift, Gradle 전체 41 tasks를 통과했다.
