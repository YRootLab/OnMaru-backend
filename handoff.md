...
- 구현 범위: PIN·UNPIN·EXCLUDE·UNEXCLUDE·proposal action, 엄격한 ExplorationSnapshot, SavedJourney 생성·목록·상세·재개·삭제의 OpenAPI 3.1 계약과 fixture 20개를 추가했다.
- 검증 경계: request/response JSON Schema와 선언 status·required path/header/query, 동일 command replay와 conflict 관계, CSRF·no-store·400/413, stateVersion 증가, 타 actor 404, label 기반 SavedJourney snapshot과 resume unavailableRefs를 전용 validator에서 검사한다.
