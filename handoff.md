...
- DataLab 단위·통합 테스트와 staging smoke Node 테스트는 작성 완료했다.
- 전체 Gradle/Node/Python/contract 검증 후 PR을 `develop` 대상으로 생성한다.
- GitHub `staging` environment에 URL, operations token, DataLab service key, read-only DB URL을 설정하고 V027/runtime 배포 후 smoke를 실행해야 한다.
- #399는 구현 PR 검증·병합 조건이 충족되면 종료할 수 있다. #392는 실제 staging smoke 성공 전까지 닫지 않는다.
