...
provider schema는 `$ref`, `$defs`, discriminator를 제거한 closed JSON Schema로 생성한다. 모든 object는 `additionalProperties: false`이며 board와 reason은 각각 최대 3개다. provider output에 tool call, URL, HTML, Markdown, control character, 임의 user-facing error field를 허용하지 않는다.
...
adversarial suite는 unknown ref 100개, duplicate/missing pin·reason, cross-candidate evidence, 빈 evidence, 초과 길이, extra field, URL/HTML/Markdown/control character와 outcome별 shape 혼합을 포함한다.
