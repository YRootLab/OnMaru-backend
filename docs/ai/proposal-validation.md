...
provider schema는 `$ref`, `$defs`, discriminator를 제거한 closed JSON Schema로 생성한다. 모든 object는 `additionalProperties: false`이며 board와 reason은 각각 최대 3개다. wire field는 `orderedRefs`, `evidenceIds`, `allowFreeText` 같은 camelCase alias만 허용하고 내부 Python field name 우회 입력을 받지 않는다. provider output에 tool call, URI scheme, protocol-relative·IP·IDN URL, HTML, Markdown 구조 문자·목록·Setext 구문, control character, 임의 user-facing error field를 허용하지 않는다.
...
adversarial suite는 unknown ref 100개, duplicate/missing pin·reason, cross-candidate evidence, 빈 evidence, 초과 길이, extra field와 snake_case alias 우회, 일반 URI scheme·protocol-relative/IP/IDN URL·HTML·Markdown·control character, outcome별 shape 혼합을 포함한다. 생성 평문 경계는 reason summary, clarification question, choice label에서 동일하게 검증한다.
