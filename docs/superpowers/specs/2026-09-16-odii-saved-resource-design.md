...
목록은 canonical REST 계약에 따라 `savedAt DESC, resourceId DESC`로 정렬한다. cursor는 member ID, resource type, limit, `asOf`, 마지막 정렬 tuple을 HMAC 서명하고 10분 뒤 만료한다. 다른 member의 cursor는 resource 존재를 숨기기 위해 404, type/limit 변경 또는 변조는 `CURSOR_INVALID`로 처리한다.
