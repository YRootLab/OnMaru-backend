...
- `gradle.lockfile`, `apps/spring-api/gradle.lockfile`: 해석된 Gradle 의존성 버전 고정.
...
- [ ] **Step 2: Wrapper를 저장소 루트에 두고 멀티프로젝트 빌드 파일을 작성한다**
...
- [ ] **Step 3: Wrapper와 project graph를 검증한다**
...
- [ ] **Step 4: Spring 테스트와 dependency lock을 검증한다**

Run: `./gradlew dependencies --write-locks && ./gradlew :apps:spring-api:test && ./gradlew check`

Expected: 모든 task가 성공하고 두 테스트가 통과한다.

- [ ] **Step 5: Spring 동작을 커밋한다**

```bash
git add apps/spring-api gradle.lockfile
...
- [ ] **Step 1: Python 프로젝트 설정과 lock을 생성한다**
