...
공개 API의 VisitReview는 `p-jeonju-hanok-village` 같은 안정적인 문자열 `placeId`, 장소명, 행정구역 코드, 좌표를 반환한다. 반면 기존 `community_visit_reviews`는 `catalog_place_identity.id` UUID만 FK로 보관한다. 공개 문자열 ID와 내부 UUID의 canonical mapping이 없으면 production write는 FK를 만족할 수 없고, 활성 Catalog revision만 매번 조인하면 장소명이 변경되거나 비공개·삭제된 뒤 과거 후기를 같은 모습으로 조회할 수 없다.

좋아요·신고·moderation은 후기와 같은 PostgreSQL transaction 경계에서 referential integrity와 중복 방지를 보장해야 한다. 후기 목록은 최대 50개를 반환하므로 항목별 Catalog 재조회(N+1)를 만들지 않아야 한다.
...
Catalog mapping은 public ID별로 하나의 내부 UUID를 가리키며, source revision 교체와 무관하게 유지한다. VisitReview write는 mapping과 현재 공개·후기 작성 가능 Catalog 상태를 한 transaction에서 확인한다. review row는 publicPlaceId, placeName, regionCode, latitude, longitude를 작성 시점 snapshot으로 보존한다. 후속 읽기는 snapshot을 기본으로 사용하며, 현재 Catalog 상태가 바뀌었다고 기존 후기의 장소 표시를 변형하지 않는다.
...
* 장점: public ID mapping은 Community가 Catalog table을 임의로 갱신하지 않고 Catalog write path가 소유한다.
...
```yaml
constraints:
  - id: visit-review-keeps-catalog-fk
    kind: required_pattern
    paths: ["apps/spring-api/src/main/resources/db/migration/baseline/V0*.sql"]
    pattern: ["community_visit_reviews", "catalog_place_identity"]
    severity: major
    message: "VisitReview persistence must retain a Catalog place UUID foreign-key relationship."
