# Foundation architecture ADR drafts

2026-09-12. **초안이며 아직 ADR 파일이 아니다.** 현재 accepted ADR은 ADR-0001 프로세스 기록과 ADR-0002의 Spring Boot/FastAPI 역할 분리다. Toolkit preflight는 기존 docs/decisions를 확인했고 related --paths docs/planning docs/database 결과0이었다. 기존 accepted 구조 ADR을 대체하는 요청은 없다.
...
공통 확인 사실: documentation-first, Java/Spring+경량FastAPI 선택, 여정/카카오저장 우선, 여정 SSE+snapshot 복구, 실제성능 미검증. 공통 가정: 단일instance/작은pilot, DB connection budget 확보 가능. 미확인: 호스팅/backup/모델budget/운영책임자. 아래 숫자는 significance이지 시스템 품질점수가 아니다.
...
- **확인 근거:** [module-boundaries.md](../../architecture/module-boundaries.md), 원본 감사 관련 F01–F21.
...
- **영향 경로:** `docs/planning/architecture-blueprint.md`, `docs/architecture/`, `docs/spring/`, `docs/ai/`, 향후 해당 core/adapter source.
...
- **확인 근거:** [identity-and-journey.md](../../spring/identity-and-journey.md), 원본 감사 관련 F01–F21.
...
- **영향 경로:** `docs/database/schema.md`, `docs/spring/`, `docs/operations/`, 향후 해당 core/adapter source.
...
## REST command, SSE notification, DB snapshot으로 지도·여정 실행을 분리한다

- **Slug:** `rest-sse-run-lifecycle`
- **문제:** SSE 연결 단절과 QUEUED 고착·늦은완료·무료자원 초과를 FE와 서버가 함께 처리해야 한다.
- **확인 근거:** [runtime-and-reliability.md](../../operations/runtime-and-reliability.md), 원본 감사 관련 F01–F21.
- **결정 제안:** 지도짧은REST, 여정202+SSE notification+GET snapshot 복구, deadline/CAS/DB admission.
- **주요 driver:** 복구 가능한 snapshot과 bounded 자원으로 단일인스턴스 운영.
- **감수할 단점:** SSE proxy/heartbeat/reconnect/replay buffer 운영과 포화시429 및 queued timeout.
- **영향 경로:** `docs/planning/journey-exploration/seven-day-mvp-fe-handoff.md`, `docs/contracts/`, `docs/operations/`, 향후 해당 core/adapter source.
- **검증:** commit-dispatch kill, cancellation race, deadline, saturation 및 FE stale response 테스트. 현재 runtime 증거 없음.
- **재검토:** SSE connection 비용이 예산을 넘거나 durable event replay·동시편집 요구가 확정될 때.
- **Significance:** 14/14; [7개 점수 입력](adr-scores/rest-polling-run-lifecycle.json)는 과거 slug의 평가 근거다.

### 검토 대안과 절충

1. **REST+SSE+snapshot**: 장점 — 진행변화를 즉시 알리고, 연결 단절은 authoritative GET으로 복구한다. 단점 — proxy/heartbeat/reconnect와 작은 replay buffer가 필요하다.

2. **REST+polling**: 장점 — snapshot 재조회와 proxy 복구가 단순하다. 단점 — 주기 GET 비용과 최대 polling 간격의 표시지연이 있다.
...
- **영향 경로:** `docs/planning/data-api-design.md`, `docs/spring/`, `docs/operations/`, 향후 해당 core/adapter source.
...
- **확인 근거:** [retrieval-and-rag.md](../../ai/retrieval-and-rag.md), 원본 감사 관련 F01–F21.
...
- **영향 경로:** `docs/planning/journey-exploration/staging-demo-release-and-success-gates.md`, `docs/ai/`, `docs/operations/`, 향후 해당 core/adapter source.
