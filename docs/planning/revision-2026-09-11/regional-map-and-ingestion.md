...
환경당 PostgreSQL 1개를 유지한다. 아래는 executable DDL 이전의 후속 스키마 계약이다. audio/insights는 원천 저장 소유권 구분이며 별도 서비스·DB를 뜻하지 않는다. UUID PK, 시각 timestamptz, 날짜 date, 원본 코드는 text가 기본이다. 원천 응답 필드 매핑의 정답은 기존 국문·오디·방문자·집중률 API 문서와 검증 fixture다. 새 원천 API 기능이 확인되었다고 주장하지 않는다.
...
수집 worker1, 전역 원천 HTTP 동시1, dataset 순차 수행, AI executor와 분리한다. 원격 대기 중 DB connection/transaction을 점유하지 않는다. dataset 실행budget30분, 원천 credential별 일 호출quota는 실제 발급 한도 확인 후 설정하며 20% 여유를 남긴다. 한도 미확인 상태는 LIVE 수집 활성화 게이트를 통과하지 못한다.
...
필수 fixture: 동일 subject/다른 issuer, 동시 로그인 고아 회원0, 서버03시 수면 후 재기동1회 등록, 마지막 page 실패 LKG 불변, 원천200오류/빈full/429 장기대기, lease 상실 후 늦은 write0, delta 게시 기존행 보존, 부모 없는 구/동명구/경계점/미지원지역, 지도 drag 본문 요청0, limit의 정확한 배수/동일createdAt/삭제 중cursor/지역 변경 늦은응답. 실제 DB·FE 테스트는 아직 없다.
