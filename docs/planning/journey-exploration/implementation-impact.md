...
2026-09-10 후속 검토: [회원·여정 상세안](journey-service-plan.md)의 JE-01~07까지 통합 대상에 추가한다. W/X graph는 여전히 기존 상태이며 아래 매핑은 실행 wave나 발행된 Issue가 아니다.

제출까지 약7일인 현재 실행 우선순위는 [7일 MVP FE 전달서](seven-day-mvp-fe-handoff.md)를 따른다. X/JE 전체를 이번 제출에 포함하지 않는다. 여정 탐색의 익명 세션·후보3개·고정·proposal 적용·카카오 로그인·저장·다시 열기만 P0로 묶고, 오디 AI·공유·현장 기록·multi-agent는 후속 graph로 남긴다.

| 추가 범위 | 기존 후보 영향 | 통합할 때 필요한 경계 |
|---|---|---|
| JE-01 설명 전용·복수 진입 | X0,X3,X4,X5 | ANSWER 결과, 지역 불필요 질문, 근거 fixture |
| JE-02 오디에서 담기 | X0,X2,X6 | ADD_RESOURCE, active run 충돌, 플레이어 연속성 |
| JE-03 관계 보기 | X0,X3,X6 | 지원 ref type, 주제 라벨, 목록 대안 |
| JE-04 회원·저장 | W0,X0,X2 및 독립 회원 작업 후보 | 인증·프로필·익명 복사·세션 해제·탈퇴; W5 온기 작성에 숨기지 않음 |
| JE-05 기기 이어보기 | X2,X6 및 회원 작업 후보 | 보드와 재생 위치 버전 분리, 저장 수명·접근 검사 |
| JE-06 읽기 전용 공유 | 독립 공유 작업 후보, X0,X6,X8 | 회원 저장 선행, 공개판·철회·비공개 원천·캐시 검증 |
| JE-07 현장·기록 | X0,X3,X6,X8 | 검수 관찰 포인트, 수동 장소 선택, 실제 방문과 구분 |

새 회원·공유 후보의 정식 ID, 선행 관계, 파일 소유권, wave는 통합 graph에서 계산한다. 회원 저장·공유가 포함된 최종 E2E는 기존 X5 완료만으로 충족되지 않는다.
