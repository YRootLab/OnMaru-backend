...
부하 목표가 현재 예상 트래픽을 의미하지 않는다. 인증 제공자와 세션 방식은 W0에서 선택한다. 제안 기본값은 조회 공개, 작성은 서버가 검증한 주체만 허용이며, guest session을 허용할지 계정을 요구할지는 제품 결정이다. 기존 localStorage를 서버 소유권 증명으로 쓰지 않는다.
...
| 작성 주체 | guest signed session vs 외부 IdP 계정. 별도 계정 관리 시스템은 보류 | W5 |
