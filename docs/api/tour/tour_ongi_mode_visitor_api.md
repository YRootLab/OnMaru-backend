# Feature: 온기모드 (OnGi Mode) — 한국관광공사 빅데이터 지역별 방문자수 API (광역/기초 지자체)

본 문서는 온마루의 '온기모드' 기능에서 **관광지 집중률 데이터와 함께 활용할 광역/기초 지자체별 실제 방문자수 데이터**의 API 명세를 정리한 문서입니다.

> 💡 **온기모드(OnGi Mode) 활용 컨셉**
> - 집중률(%) API가 "지금 해당 관광지가 얼마나 혼잡한가(예측/실시간)"를 보여준다면, 본 방문자수 API는 "실제로 며칠에 몇 명이 방문했는가(과거 추이)"를 보여줍니다.
> - 두 데이터를 조합함으로써 유저에게 "보통 이 지역은 평균 00만 명이 방문하는데, 현재 혼잡도는 00%입니다"와 같은 구체적이고 직관적인 정보를 제공할 수 있습니다.
...
1. **[광역지자체] GET `/metcoRegnVisitrDDList`**
   - 광역지자체(시/도) 단위의 일자별 방문자수 집계 데이터 정보 조회
2. **[기초지자체] GET `/locgoRegnVisitrDDList`**
   - 기초지자체(시/군/구) 단위의 일자별 방문자수 집계 데이터 정보 조회

---

## 3. 요청 파라미터 (Request Parameters)

두 엔드포인트는 유사한 요청 파라미터를 사용합니다.

| 파라미터 | 타입 | 필수 | 설명 | 예시 |
|---|---|---|---|---|
| `serviceKey` | string | ✅ | 공공데이터포털 인증키 | (발급받은 디코딩/인코딩 키) |
...
| `areaCd` | string | | 광역지자체 코드 (시/도 단위 필터링 시 사용) | `11` (서울), `51` (강원) 등 |
| `signguCd` | string | | 기초지자체 코드 (기초지자체 조회 시 시군구 필터링) | `11110` (종로구) |
...
## 4. [광역지자체] GET `/metcoRegnVisitrDDList` 명세
...
## 5. [기초지자체] GET `/locgoRegnVisitrDDList` 명세
...
1. **방문자 정의**: 이동통신 데이터 기반 추정치이므로 실제 매표소 입장 인원 등과는 상이할 수 있습니다.
2. **순방문자 집계**: 일자별 순방문자 기준이므로 한 명이 하루 동안 동일 지자체를 여러 번 오가도 1명으로 집계되나, 2박 3일 방문 시에는 각각의 날짜별로 1명씩(총 3명) 집계됩니다.
...
4. **일 데이터 업데이트 주기**: 실시간 데이터가 아니며 보통 전일 혹은 수일 전의 집계 데이터가 업로드됩니다.
