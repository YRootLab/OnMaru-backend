...
```dbml
// 1. 관광지 (Tour Spot) 테이블
// 한국관광공사 오디(Odii) API의 관광지(Theme) 정보를 저장합니다.
Table tour_spots {
  tid varchar [note: "관광지 ID"]
  tlid varchar [note: "관광지 언어 ID"]
  lang_code varchar [note: "언어 코드 (예: ko)"]
  ...
  indexes {
    (tid, tlid) [pk] // 복합 키를 PK로 사용하거나, 별도 UUID를 둘 수 있습니다.
  }
}

// 2. 오디오 가이드 (Audio Guide / Story) 테이블
// 특정 관광지에 속한 개별 이야기(도슨트) 정보를 저장합니다.
Table audio_guides {
  ...
  audio_title varchar [note: "오디오 제목 (가이드 이름)"]
...
// 관계 (Relationships)
// 하나의 관광지는 여러 개의 오디오 가이드(이야기)를 가집니다.
