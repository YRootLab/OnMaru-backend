...
// V027은 catalog_datalab_region_mappings를 source-code row의 DataLab 전용 1:1 registry로
// 추가한다. 내부 지역과 DataLab code의 중복, SIDO/SIGUNGU 계층·parent 불일치, 불완전한
// ACTIVE provenance를 DB에서 거부한다. PENDING/REJECTED mapping은 감사 이력에는 남지만
// 운영 수집 대상으로 선택되지 않는다.
